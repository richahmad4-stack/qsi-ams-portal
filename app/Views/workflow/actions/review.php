...
(() => {
    ...
    const calculate = () => {
        ...
        if (basis) {
            basis.value = `Auto calculated using MD-style employee bands and scheme rules. Effective employees: ${employees}. Standard basis: ${standardDays.map((days, index) => `${standards[index] || 'GENERAL'}: ${days.toFixed(2)} days`).join('; ')}. Adjustments: ${factors.length ? factors.join('; ') : 'none'}. Formula reference: Initial audit days = round to nearest 0.5 day of [(highest standard base days + 20% of each additional standard base days) x adjustment factor]. Stage 1 = 1 day when total <= 3 days, otherwise about 25% of total capped at 2 days. Stage 2 = total initial audit days - Stage 1. Surveillance = about one-third of initial audit days. Recertification = about two-thirds of initial audit days. Initial audit: ${total.toFixed(2)} days, split Stage 1 ${stage1.toFixed(2)} / Stage 2 ${stage2.toFixed(2)}.`;
        }
    };
